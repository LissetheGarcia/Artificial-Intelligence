...
def build_arg_parser():
    ...
    parser.add_argument("--output-file", dest="output_file", required=True, help="Output filename (without the extension) where the point cloud will be saved")
...
if __name__ == '__main__':
    ...
    stereo = cv2.StereoSGBM(minDisparity = min_disp, numDisparities = num_disp, SADWindowSize = win_size, uniquenessRatio = 10, speckleWindowSize = 100, speckleRange = 32, disp12MaxDiff = 1, P1 = 8*3*win_size**2, P2 = 32*3*win_size**2, fullDP = True)
    ...
    Q = np.float32([[1, 0, 0, -w/2.0], [0,-1, 0, h/2.0], [0, 0, 0, -focal_length], [0, 0, 1, 0]])
